      ...
      <div className="mx-auto grid max-w-2xl gap-6">
        <SurfaceCard className="p-7">
          <div className="flex items-center gap-4 border-b border-border pb-6">
            <UserAvatar initials={currentUser.initials} className="size-16 text-xl" />
            <div>
              <h2 className="text-xl font-bold text-foreground">{currentUser.name}</h2>
              <p className="flex items-center gap-1 text-sm font-medium text-success">
                <BadgeCheck className="size-4" aria-hidden /> Verified account
              </p>
            </div>
          </div>
        ...
        <SurfaceCard className="p-6">
          <SectionHeading
            ...
            description={`Level ${kycProfile.level} · daily limit ${formatBDT(kycProfile.limitPerDay)}`}
          />
          <p className="text-sm text-muted-foreground">
            Verify your NID, passport or driving licence to raise your limits and unlock higher withdrawals.
          </p>
          <Link to="/kyc" className="mt-4 block">
            <SubmitButton className="w-full">Verify my identity</SubmitButton>
          </Link>
        </SurfaceCard>
        ...
        <SurfaceCard className="p-6">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <Link to="/settings">
              <SoftButton className="w-full">Settings</SoftButton>
            </Link>
            <Link to="/sources">
              <SoftButton className="w-full">Payment sources</SoftButton>
            </Link>
          </div>
        </SurfaceCard>
      </div>
